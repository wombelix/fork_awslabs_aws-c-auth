...
struct aws_string *sso_access_token_path(struct aws_allocator *allocator, const struct aws_string *sso_start_url) {
...
static int s_load_access_token_from_file(
    ...
    if (aws_byte_buf_init_from_file(&file_contents, allocator, aws_string_c_str(token_path))) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER, "sso: failed to load access token file %s", aws_string_c_str(token_path));
        goto done;
    }
    ...
    if (document_root == NULL) {
        AWS_LOGF_ERROR(
            ...
            "sso: failed to parse access token file %s",
            aws_string_c_str(token_path));
        goto done;
    }
    ...
    if (!aws_json_value_is_string(access_token) ||
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER, "sso: failed to parse accessToken from %s", aws_string_c_str(token_path));
        goto done;
    }
    ...
    if (!aws_json_value_is_string(expires_at) ||
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER, "sso: failed to parse expiresAt from %s", aws_string_c_str(token_path));
        goto done;
    }
...
static struct sso_user_data *s_user_data_new(
    ...
    struct sso_user_data *wrapped_user_data = aws_mem_calloc(sso_provider->allocator, 1, sizeof(struct sso_user_data));
 ...
 *   }
 * }
 * This is similar to aws_parse_credentials_from_aws_json_object, but differs in the format of the
...
static struct aws_credentials *s_parse_credentials_from_response(
    ...
    if (document_root == NULL) {
        AWS_LOGF_ERROR(AWS_LS_AUTH_CREDENTIALS_PROVIDER, "Failed to parse Json Response.");

        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER, "(id=%p) sso: failed to parse JSON response", (void *)user_data);
        goto done;
    }
    ...
    if (!aws_json_value_is_object(role_credentials)) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: failed to extract roleCredentials from JSON response",
            (void *)user_data);
        goto done;
    }
    ...
    if (!aws_json_value_is_string(access_key) ||
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: failed to extract accessKeyId from JSON response",
            (void *)user_data);
        goto done;
    }
    ...
    if (!aws_json_value_is_string(secret_key) ||
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: failed to extract secretAccessKey from JSON response",
            (void *)user_data);
        goto done;
    }
    ...
    if (!aws_json_value_is_string(session_token) ||
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: failed to extract sessionToken from JSON response",
            (void *)user_data);
        goto done;
    }
    ...
    if (!aws_json_value_is_number(expiration) ||
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: failed to extract expiration from JSON response",
            (void *)user_data);
        goto done;
    }
    ...
    if (credentials == NULL) {
        ...
        AWS_LOGF_DEBUG(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: new credentials expire at %" PRIu64,
            (void *)user_data,
...
static void s_finalize_get_credentials_query(struct sso_user_data *user_data) {
    ...
    if (credentials == NULL) {
        ...
        AWS_LOGF_WARN(
            ...
            (void *)user_data,
            aws_error_str(user_data->error_code));
    }
...
static int s_on_incoming_body_fn(
    ...
    if (body->len + user_data->response.len > SSO_RESPONSE_SIZE_LIMIT) {
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) SSO credentials provider query response exceeded maximum allowed length",
            (void *)user_data->sso_provider);
    ...
    if (aws_byte_buf_append_dynamic(&user_data->response, body)) {
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) SSO credentials provider query error appending response: %s",
            (void *)user_data->sso_provider,
...
static int s_on_incoming_headers_fn(
    ...
    if (header_block == AWS_HTTP_HEADER_BLOCK_MAIN && user_data->status_code == 0) {
        ...
        if (impl->function_table->aws_http_stream_get_incoming_response_status(stream, &user_data->status_code)) {
            AWS_LOGF_ERROR(
                AWS_LS_AUTH_CREDENTIALS_PROVIDER,
                "(id=%p) o: failed to get http status code: %s",
                (void *)user_data->sso_provider,
                aws_error_str(aws_last_error()));
            return AWS_OP_ERR;
        }
        AWS_LOGF_DEBUG(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) SSO credentials provider query received http status code %d",
            (void *)user_data->sso_provider,
...
static void s_on_stream_complete_fn(struct aws_http_stream *stream, int error_code, void *data) {
    ...
    if (error_code || user_data->status_code != AWS_HTTP_STATUS_CODE_200_OK) {
        if (error_code || s_is_retryable_http_response_code(user_data->status_code)) {
            ...
            AWS_LOGF_WARN(
                AWS_LS_AUTH_CREDENTIALS_PROVIDER,
                "id=%p: SSO provider failed to schedule retry %d(%s)",
                (void *)user_data->sso_provider,
                error_code,
        ...
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "id=%p: SSO provider failed to register retry success %d(%s)",
            (void *)user_data->sso_provider,
            error_code,
            aws_error_str(error_code));
    }
...
static void s_on_acquire_connection(struct aws_http_connection *connection, int error_code, void *data) {
    ...
    if (connection == NULL) {
        AWS_LOGF_WARN(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "id=%p: sso provider failed to acquire a connection, error code %d(%s)",
            (void *)user_data->sso_provider,
            error_code,
            aws_error_str(error_code));
...
static void s_on_retry_token_acquired(
    ...
    if (error_code) {
        AWS_LOGF_WARN(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p) sso: failed to acquire retry token: %s",
            wrapped_user_data,
            aws_error_str(error_code));
        user_data->error_code = error_code;
...
static void s_sso_credentials_provider_destroy(struct aws_credentials_provider *provider) {
    ...
    aws_string_destroy(impl->token_path);
...
static int s_construct_endpoint(
    ...
    struct aws_byte_cursor c_cn = aws_byte_cursor_from_c_str(".cn");

    if (aws_byte_buf_init(endpoint_buf, allocator, c_prefix.len + c_region.len + c_suffix.len)) {
        return AWS_OP_ERR;
    }

    if (aws_byte_buf_append(endpoint_buf, &c_prefix) || aws_byte_buf_append(endpoint_buf, &c_region) ||
    ...
    if (aws_string_eq_c_str_ignore_case(region, "cn-north-1") ||
        ...
        if (aws_byte_buf_append_dynamic(endpoint_buf, &c_cn)) {
            return AWS_OP_ERR;
        }
    }
...
static void s_parameters_destroy(struct sso_parameters *parameters) {
    ...
    aws_string_destroy(parameters->region);
    aws_string_destroy(parameters->role_name);
    aws_string_destroy(parameters->start_url);
...
static struct sso_parameters *s_sso_parameters_new_from_profile(
    struct aws_allocator *allocator,
    const struct aws_credentials_provider_sso_options *sso_options) {
    ...
    config_file_path = aws_get_config_file_path(allocator, &sso_options->config_file_name_override);
    if (!config_file_path) {
        AWS_LOGF_DEBUG(AWS_LS_AUTH_CREDENTIALS_PROVIDER, "sso: failed to resolve config file path");
        goto done;
    }

    profile_name = aws_get_profile_name(allocator, &sso_options->profile_name_override);
    if (!profile_name) {
    ...
    if (!sso_start_url || !sso_region || !sso_account_id || !sso_role_name) {
        AWS_LOGF_DEBUG(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "sso: invalid configuration for \"%s\" profile",
            aws_string_c_str(profile_name));
        goto done;
    }

    parameters = aws_mem_calloc(allocator, 1, sizeof(*parameters));
    ...
    parameters->region = aws_string_new_from_string(allocator, aws_profile_property_get_value(sso_region));
    parameters->role_name = aws_string_new_from_string(allocator, aws_profile_property_get_value(sso_role_name));
    parameters->start_url = aws_string_new_from_string(allocator, aws_profile_property_get_value(sso_start_url));
    token_path = sso_access_token_path(allocator, parameters->start_url);
    if (!token_path) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER, "Failed to resolve token file path during sso initialization.");
        goto done;
    }
...
struct aws_credentials_provider *aws_credentials_provider_new_sso(
    ...
    if (!options->tls_ctx) {
        AWS_LOGF_ERROR(AWS_LS_AUTH_CREDENTIALS_PROVIDER, "a TLS context is necessary for querying SSO");
        aws_raise_error(AWS_ERROR_INVALID_ARGUMENT);
        return NULL;
    }
    ...
    // TODO: why use s_default_function_table
    ...
    parameters = s_sso_parameters_new_from_profile(allocator, options);
    if (!parameters) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "(id=%p): failed to fetch sso paramters from profile with error %s",
            (void *)provider,
